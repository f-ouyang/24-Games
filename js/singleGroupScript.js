...
// The following constants are used for touch interface.
let selectedButtonID = null;// ID of the selected button
let selectionTimeout = null;//timer ID
const SELECTION_TIMEOUT_MS = 5000; // 5 seconds
// test test const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
...
// Install document listeners for mobile interface.
if (isTouchDevice) {
    document.addEventListener('click', function(event) {
            ...
            if (!clickedElement.classList.contains('numberButton') && 
                !clickedElement.classList.contains('targetButton')) {
                clearSelection();
            }
        });
} 
...
class targetButtonJavaClass extends HTMLButtonElement {
    ...
    constructor() {
        ...
        if (isTouchDevice) {
          this.addEventListener('click', handleTargetButtonTap);
        } 
        ...
        if (!isTouchDevice) {
            this.disabled = true; // Disable the button by default
        } else {
